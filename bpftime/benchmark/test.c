...
// 1. 硬编码采样阈值 (例如 1000 次触发 1 次全量观测)
// 你可以通过修改这个宏来测试不同采样率下的吞吐量变化
#define SAMPLE_THRESHOLD 5000

static __thread uint32_t sample_counter = 0;

__attribute_noinline__ uint64_t payload_logic(char *a, int b, uint64_t c) {
    volatile unsigned int x = a[b];
    volatile unsigned int y = c;
	int iter_num = 220;
    for (int i = 0; i < 220; i++) {
        x = (x * 31 + y) ^ (unsigned)i;
...
__attribute_noinline__ uint64_t __bench_uprobe(char *a, int b, uint64_t c) {
    return payload_logic(a, b, c);
}

// 2. 函数签名与 __bench_uprobe 保持绝对一致，完美适配 Benchmark 框架
__attribute_noinline__ uint64_t __bench_sample(char *a, int b, uint64_t c) {
    uint32_t trigger = 0;

    asm volatile (
        "pushfq\n\t"                 
        "incl %[counter]\n\t"        
        // 注意这里的 %[n]：因为使用了 "i" 约束，编译器会将其直接翻译为 "cmpl $1000, memory"
        "cmpl %[n], %[counter]\n\t"  
        "jb 1f\n\t"                  
        "movl $0, %[counter]\n\t"    
        "movl $1, %[trigger]\n\t"    
        "1:\n\t"
        "popfq\n\t"                  
        : [counter] "+m" (sample_counter), [trigger] "+r" (trigger)
        // 3. 将输入约束从 "r" (寄存器) 改为 "i" (立即数常量)
        : [n] "i" (SAMPLE_THRESHOLD) 
        : "cc", "memory"
    );

    if (trigger) {
		// printf("sample trigger!!!\n");
        return __bench_uprobe(a, b, c);
    } else {
        return payload_logic(a, b, c);
    }
}

// 自定义的 __bench_uprobe 函数
// __attribute_noinline__ uint64_t __bench_uprobe(char *a, int b, uint64_t c)
// {
//     volatile unsigned int x = a[b];
//     volatile unsigned int y = c;
...
// __bench_uprobe 已经在上面手动定义了，不再使用宏
...
void *run_bench_functions(void *id_ptr)
{
	int id = *(int *)id_ptr;
	printf("id: %d\n", id);
	// Only run __bench_uprobe test
	// do_benchmark_func(__bench_uprobe, iter, id);
	do_benchmark_func(__bench_sample, iter, id);
	return NULL;
}
